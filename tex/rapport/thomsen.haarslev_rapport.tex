...
In the following, we will make an analysis of how well our translated REs perform on various RE implementations compared to the performance of scan-for-matches. In the analysis, we will show various plots of our performance results. For some of our tests, the REs become large enough, that limits in some of the RE implementations stops them from compiling the REs. When this happens, our test returns a match time of 0. This means that in some of our graphs, if it says it took zero milliseconds to match, it is because it didn't find any results. In some of these cases, results could actually be found, but the RE implementation took too long (we set a limit of 120 seconds for matching), and we stopped the process. We did this because if a given RE implementation is already orders of magnitude slower than scan-for-matches, it doesn't really matter how many orders of magnitude.

\subsubsection{Mismatches}

In figure~\ref{graph:cases:mismatches} we look at how well the different engines matches patterns with mismatches. For our control, scan-for-matches,  the match time doesn't seem affected by the complexity of mismatches-patterns at all, and shows great performance for any level of complexity. 

Ruby shows poor performance, with a match time that explodes with the complexity of the pattern, and then quickly drops to zero match time, which means that the pattern was either too large for Ruby to compile, or matching was slower than the limit we had set for the test.

RE2 is showing surprisingly good results for patterns of low complexity. RE2 actually shows better performance than scan-for-matches for a few of the test patterns, but it seems like the match time quickly rises, and then either gives up on compiling the patterns, or spends too long on the matching. Interestingly, this happens at the same level of complexity as Ruby.
...
We expected these tests to have poorer performance than mismatches and deletions, as insertions are the greatest factor in calculating how large the translated RE becomes. Overall, the test shows what we expected; that the performance of both RE2 and Ruby is far inferior to scan-for-matches, that once again isn't affected by the complexity of the pattern. Surprisingly, RE2 once again proves to be performing better than scan-for-matches for low-complexity patterns. However, like all tests before, both RE2 and Ruby fails when a certain complexity is reached.
...
In figure~\ref{graph:cases:ranges}, we look at ranges and how the size of the range affects the match time for all three engines. Contrary to our previous tests, both RE2 and Ruby shows superior performance to scan-for-matches, when it comes to ranges. This is likely caused by the fact, that ranges aren't translated into enormous REs like the combination patterns. In the previous tests, we also how RE2 would outperform scan-for-matches, if the complexity of the pattern is small.

Additionally, the size of the range doesn't seem to make a difference for either of the three engines, in terms of match time. 
...
Similarly to ranges, sequences are simple patterns when translated into REs (In fact, they don't change at all). Much like ranges, here we also see that RE2 and Ruby performs a lot better than scan-for-matches, further suggesting that RE2 and Ruby are superior to scan-for-matches when it comes to low-complexity patterns.

Interestingly, RE2 does a lot better than Ruby for smaller patterns, but as the patterns become larger, Ruby starts to perform better than RE2. Larger sequences yield fewer matches than smaller sequences, which suggests that Ruby performs bad when it finds a lot of matches, but performs well when there are fewer matches, while RE2 shows the opposite behaviour.
...
We presented a way to recursively translate patscan patterns into REs. By doing so, we were able to analyse the performance of scan-for-matches against various RE implementations, to see if it could prove advantageous to run the patscan patterns on RE implementations.

From our tests, we were able to determine, that in a few cases, translating patscan patterns into REs is superior to scan-for-matches. This happens for very simple patterns, such as sequences, ranges, or very simple combination patterns. For some of these patterns, translating into a RE gives a match time more than 10 times faster than scan-for-matches.

Unfortunately, the true power of scan-for-matches lies with being able to match complex combination patterns in relatively low time. For such patterns, scan-for-matches far out-performs our REs on all RE implementations.

In conclusion, for a narrow problem-margin, translating patscan patterns into REs, for the purpose of achieving a faster match time, is advantageous. However, for most patterns, scan-for-matches will have a far superior match time.
