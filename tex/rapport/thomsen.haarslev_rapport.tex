...
Scan-For-Matches is the state-of-the-art open-source solution for DNA sequence analysis, but is no longer being maintained. We present an algorithm for translating Scan-For-Matches patterns into regular expressions. By translating patscan patterns into regular expressions, we are able to measure the performance of searching through genomic data on different string search engines in the form of regular expression implementations. We generate a set of regular expressions and analyze their performance, looking at the complexity of the original scan-for-matches patterns, and the time it takes to find matches. We discover that our regular expressions run faster than scan-for-matches for sequences, ranges and low-complexity 'combination' patterns. As complexity of the scan-for-matches 'combination' patterns increases, scan-for-matches shows superior performance.
