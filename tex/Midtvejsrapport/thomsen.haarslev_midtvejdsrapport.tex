...
\subsubsection{Grammar (TO BE REWRITTEN)}
...
\begin{algorithm}
	\caption{find\_combinations
  	...
  	\begin{algorithmic}[1]
    		\Require
    			\Statex \textbf{seq} - sequence string to be translated
    			\Statex \textbf{$m_{max}$} - allowed number of mismatches
    			\Statex \textbf{$i_{max}$} - allowed number of insertions
    			\Statex \textbf{$d_{max}$} - allowed number of deletions
    		\Ensure
		...
		\Statex
    		...
    		\ForAll{LL \In left\_tree} \Comment{LL: Left leaf}
    			\ForAll{RL \textbf{in} right\_tree} \Comment{RL: Right leaf}
    				\If{$LL.m + RL.m > m_{max}$ \Or $LL.i + RL.i > i_{max}$ \Or $LL.d + RL.d > d_{max}$}
    					\State continue
    				\EndIf
